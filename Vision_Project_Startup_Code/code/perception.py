...
def color_thresh_rock(img, rgb_thresh=(100, 100, 20)): #yellow isn't pure yellow
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
...
#increase map fidelity by
# 1. limit camera measurments to only capture points less than distance 8 meters
...
def perception_step(Rover, debug):
    ...
    #STEP 5
    #PIX
    x_thresh , y_thresh = rover_coords(threshed)      #navigable Terrain pix
    ...
    scale = 6
    x_world , y_world = pix_to_world(x_thresh,y_thresh,Rover.pos[0],Rover.pos[1],Rover.yaw,Rover.worldmap.shape[0],scale)
    xw_rock, yw_rock = pix_to_world(x_rock,y_rock,Rover.pos[0],Rover.pos[1],Rover.yaw,Rover.worldmap.shape[0],scale)
    xw_obst , yw_obst = pix_to_world(x_obst,y_obst,Rover.pos[0],Rover.pos[1],Rover.yaw,Rover.worldmap.shape[0],scale)
    ...
    rock_dists, rock_angles = to_polar_coords(x_rock, y_rock)
